...
| `ascendc-docs-search` | Migrated and redesigned | Hard-coded one A3/CANN 8.5 environment, repository paths, and file counts; used Claude `Task`/`Explore`; online clients depended on an unstable private endpoint; `requirements.txt` listed only `requests` while the fetcher also imported Beautiful Soup; one copied cleaner referenced a license file absent from TileXR | Added dynamic root/version discovery and official-primary-source fallback; removed the online client, fetcher, cleaner, and dependency file; added reusable local search patterns |
...
| `commit-push-pr` | Not migrated | Duplicates the marketplace's Git-delivery workflow and the installed skill namespace; instructs the agent to stage, push, and open a PR in one response with unsafe parallelism | Continue to use `superpowers-neo-git-delivery`; no duplicate skill is published |
...
- absence of Claude-specific task instructions, initializer placeholders, removed online-search dependencies, and the duplicate `commit-push-pr` skill;
