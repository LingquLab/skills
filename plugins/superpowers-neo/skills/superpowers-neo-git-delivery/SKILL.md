...
5. Confirm relevant verification is fresh and applies to the exact content being delivered. When a commit is authorized, fix failures caused by the task. When commit is not authorized, do not edit the workspace to repair a failing check or hook; report the failure and stop if it blocks the named action. If a check is unavailable or an unrelated failure remains, report the exact gap and residual risk before committing or pushing.
...
|---|---|---|---|
| Branch create/switch | Ask only when a branch decision, creation, or switch is needed. Unresolved ownership on the current non-default branch limits automatic push, not the default scoped commit. | Create or switch only when requested, using the named target or repository conventions. | Select an appropriate task branch or create and switch to one without another prompt. |
...
| Pull request | Require explicit instruction or approval of a named plan action. | Reuse an open PR for the exact head branch; otherwise create the requested PR only after readiness checks against that committed head. Infer no branch creation, commit, or merge. | Reuse an open PR for the selected head branch; otherwise create one after the task branch is pushed and the committed head is verified. |
