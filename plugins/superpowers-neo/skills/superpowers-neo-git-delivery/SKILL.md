...
Distinguish automatic entry from manual invocation:
...
- **Manual invocation:** When the user invokes this skill through `$superpowers-neo:superpowers-neo-git-delivery` syntax or an equivalent explicit skill attachment, treat that invocation as one bundled authorization to select or create and switch to a task branch, commit the exact task scope, push that branch normally, and create a pull request. Do not ask separately for those four actions. A mere automatic trigger, quoted skill name, or discussion of the skill is not manual invocation.

An explicit opt-out or narrower instruction overrides either mode. Neither mode authorizes merge, history rewrite, force push, hook bypass, cleanup, discarding changes, or bypassing repository protections.
...
3. Prefer an appropriate existing task branch. Under manual invocation, create and switch to a new task branch without another prompt when no appropriate branch exists; follow user or repository conventions, otherwise use `codex/<topic>` with lowercase hyphenated words. Under automatic entry, ask before creating or switching branches when on the default branch. Never carry unrelated commits or move, stash, or discard user changes to make a branch switch work; stop and ask if isolation cannot be achieved safely.
...
6. Determine and report the branch, file scope, commit message, verification evidence, and known risks. Proceed with the scoped commit unless the user opted out. Under automatic entry on the default branch, first obtain the branch decision required above. Under manual invocation, make the branch decision within the bundled authority and continue without redundant authorization prompts.
...
| Action | Automatic entry | Manual invocation |
|---|---|---|
| Branch create/switch | Ask on the default branch or when branch ownership is unresolved. | Select an appropriate task branch or create and switch to one without another prompt. |
| Commit | Commit the exact completed task scope after checks. | Commit the exact completed task scope after checks. |
| Push | Push only an established, task-owned non-default branch; otherwise ask. | Normally push only the selected task branch and set upstream on its first push. |
| Pull request | Require explicit instruction or approval of a named plan action. | Create a pull request after the task branch is pushed. |
| Merge | Require a separate explicit request after checking CI, review status, branch currency, and repository policy. | Same. |
| Local history rewrite | Require a separate explicit request after identifying affected commits and collaboration impact. | Same. |
| `--force-with-lease` | Require separate explicit confirmation after explaining overwrite risk. | Same. |
| Cleanup | Require an explicit request or approval of a named cleanup step covering the specific branch or worktree. | Same. |
