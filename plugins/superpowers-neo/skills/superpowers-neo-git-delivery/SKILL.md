...
6. Determine and report the branch, file scope, commit message, verification evidence, and known risks. Proceed with the scoped commit unless the user opted out. Under automatic entry on the default branch, first obtain the branch decision required above. Under an explicit named request, perform the named actions without asking again but do not infer unnamed actions. Under manual invocation, make the branch decision within the bundled authority and continue without redundant authorization prompts.
...
| Pull request | Require explicit instruction or approval of a named plan action. | Create the requested PR after readiness checks; infer no branch creation, commit, or merge. | Create a pull request after the task branch is pushed. |
